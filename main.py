...
async def get_etf_history(ticker: str, period: str = "1y"):
    try:
        # 1. 自動轉換台灣股票代碼格式
        yf_ticker = ticker
        if ticker.isdigit():
        ...
        # 3. 【核心修復】徹底拍扁 yfinance 新版產生的 MultiIndex 多級欄位
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        ...
        # 5. 【核心修復】防禦性命名相容：若索引叫 Datetime 則自動更名為 Date
        if 'Datetime' in df.columns and 'Date' not in df.columns:
            df = df.rename(columns={'Datetime': 'Date'})
            
        if 'Date' not in df.columns:
            return safe_json({"status": "error", "message": "資料格式錯誤：找不到日期欄位"})
            
        chart_data = []
        for _, row in df.iterrows():
            # 6. 【核心修復】安全提取並精準格式化日期為 YYYY-MM-DD 字串
            dt_val = row['Date']
            try:
            ...
            # 安全取值輔助函式：排除 numpy 的多重索引包裝、NaN、無窮大數值
            def _get_safe_float(col):
                if col not in row:
                    return None
                val = row[col]
                if hasattr(val, 'iloc'):
            ...
            o = _get_safe_float('Open')
            h = _get_safe_float('High')
            l = _get_safe_float('Low')
            c = _get_safe_float('Close')
            v = _get_safe_float('Volume')

            # 7. 【核心修復】Lightweight Charts 要求 K 線四大價格均不可為 null 或小於等於 0
        ...
        # 8. 【核心修復】強制依據時間進行「嚴格升序排序」，滿足前端圖表引擎要求
        chart_data = sorted(chart_data, key=lambda x: x['time'])
        
        if not chart_data:
            return safe_json({"status": "error", "message": "經清洗後無有效歷史走勢資料"})
            
        return safe_json({"status": "success", "data": chart_data})
        
    except Exception as e:
        logger.error(f"獲取歷史資料失敗: {str(e)}")
        return safe_json({"status": "error", "message": f"伺服器錯誤: {str(e)}"})
